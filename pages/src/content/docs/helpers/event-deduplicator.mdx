...
<Tabs>
  ...
  <TabItem label="JSR">
    ```typescript
    import { EventDeduplicator } from '@otoneko1102/mixi2-js/helpers';
    ```
  </TabItem>
</Tabs>
...
<Aside type="tip">
`maxAge` のデフォルトは Webhook のタイムスタンプ許容ズレ（5 分）に合わせています。通常はデフォルト値のままで問題ありません。
</Aside>
...
<Aside type="note">
`EventDeduplicator` はインメモリでイベント ID を管理します。プロセスを再起動すると記録はリセットされます。再起動直後に Webhook のリトライが届く場合は、データベース等の永続ストレージを使った独自実装を検討してください。
</Aside>

<Aside type="note">
`EventDeduplicator` は SDK 独自の拡張機能であり、公式 API 仕様には含まれません。公式の仕様については [mixi2 Developer Platform](https://developer.mixi.social/docs) を参照してください。
</Aside>
