...
<Tabs>
  <TabItem label="npm">
    ```typescript // ESM import {TextSplitter} from 'mixi2-js/helpers'; // CJS const {TextSplitter}{" "}
    = require('mixi2-js/helpers'); ```
  </TabItem>
  <TabItem label="JSR">
    ```typescript import {TextSplitter} from '@otoneko1102/mixi2-js/helpers'; ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="npm">
    ```typescript import {Client} from 'mixi2-js'; import {TextSplitter} from 'mixi2-js/helpers';
    ```
  </TabItem>
  <TabItem label="JSR">
    ```typescript import {Client} from '@otoneko1102/mixi2-js'; import {TextSplitter} from
...
<Aside type="note">
  `TextSplitter` は SDK 独自の拡張機能であり、公式 API 仕様には含まれません。公式の仕様については
