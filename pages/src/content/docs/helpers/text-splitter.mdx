...
<Aside type="note">
`TextSplitter` は JavaScript の `string.length`（UTF-16 コード単位）で文字数を計算します。mixi2 サーバー側の文字数カウント方法と完全に一致しない場合があります。
</Aside>

<Aside type="note">
`TextSplitter` は SDK 独自の拡張機能であり、公式 API 仕様には含まれません。公式の仕様については [mixi2 Developer Platform](https://developer.mixi.social/docs) を参照してください。
</Aside>
